...
import { createPinchState, pinchEnd, pinchMove, pinchStart, touchDist } from './pinch.js';
...
export class MultiPlaceSession {
	...
	async start() {
		...
		const sessionInit = {
			...
			// (./estimated-lighting.js); optional so devices without it fall
	...
	async _handleSelect() {
		...
		try { navigator.vibrate?.(15); } catch { /* haptics are a bonus, never a failure path */ }
	...
	release(group) {
		...
		try { p.anchor?.delete?.(); } catch { /* the session already released it */ }
		if (p.shadow) {
	...
	_handleEnd() {
		...
		if (this._hitTestSource) {
			try { this._hitTestSource.cancel(); } catch { /* the session already tore it down */ }
			this._hitTestSource = null;
		}
