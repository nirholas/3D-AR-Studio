// Pure scene math and (de)serialization for the AR Studio.
//
...
/** Placement scale bounds: mirrors the pinch clamp in ./pinch.js. */
...
/**
 * Full-fidelity share URL: the ?src= list (for link previews and old clients)
 * plus the `#s=` arrangement hash. Falls back to the plain model list when the
 * payload would push the link past QR-friendly length.
 *
 * @param {string} baseUrl  The page hosting the studio, e.g. 'https://acme.com/ar/'.
 * @param {Array<{src:string,title?:string,x?:number,z?:number,yaw?:number,scale?:number}>} placements
...
/**
 * The shareable URL that reopens the current scene's MODELS (sources only
 * transforms ride in the hash above). Capped so the QR stays scannable.
 *
 * @param {string} baseUrl  The page hosting the studio.
 * @param {Array<{src:string,title?:string}>} placements
 * @param {number} [max]  How many models to embed in the link.
 * @returns {string}
 */
export function studioShareUrl(baseUrl, placements, max = 4) {
	const base = String(baseUrl || '').split('#')[0];
	const list = (Array.isArray(placements) ? placements : [])
		.map((p) => ({ src: normalizeGlbUrl(p.src), title: String(p.title || '').slice(0, 60) }))
		.filter((p) => p.src);
	// De-duplicate by source: five copies of one crate share a single URL entry.
	const seen = new Set();
