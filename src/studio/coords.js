// Pure coordinate and room-key helpers for the shared AR Studio. DOM-free and
 ...
 * Accepts a bare code or a pasted ?room=CODE URL. Upper-cases, drops
...
/**
 * Build the shareable join URL for a room code, against the page hosting the
 * studio.
 * @param {string} baseUrl e.g. 'https://acme.com/ar/'
 * @param {string} code
 */
export function roomShareUrl(baseUrl, code) {
	...
	if (!c) return base;
	return `${base}${base.includes('?') ? '&' : '?'}room=${c}`;
}
